...
**Durable replication queue.** Writes that fail to replicate even after retries are appended to an on-disk log, not just held in memory. If the leader restarts, it reloads that log on startup and resumes trying to deliver them — verified by `TestPendingQueueSurvivesRestart`.
...
- Compute normalization bounds from historical data instead of hardcoding them